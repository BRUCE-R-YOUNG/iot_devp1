...
def device_main():
    ...
    while True:
        ...
        temp= float(format(bme280.temperature))
        humi= float(format(bme280.humidity))
        pre = float(format(bme280.pressure))
        
        # 光センサ       
        tsl2572.single_auto_measure()
        # 条件を自動で調整しながら1回測定を行い, luxに結果を入れる
        lux = float(format(tsl2572.illuminance))
        ...
        payload = {"DEVICE_NAME": device_name, "TIMESTAMP": now, "TEMPERATURE": int(temp), "HUMIDITY": int(humi), "PRESSURE": int(pre), "LOACTION": str(local), "LUX": int(lux)}
        logger.debug("  payload: %s", payload)
